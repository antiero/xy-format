...
| +0x06 | **track scale** (0x01=½, 0x03=1, 0x05=2, 0x06=3, 0x07=4, 0x08=5, 0x09=6, 0x0A=7, 0x0B=8, 0x0E=16). The odd values are the contiguous enum between corpus-pinned anchors and are exposed by firmware 1.1.25; retain E0 until a device-saved odd-scale capture is added. | u20–u22, `spaceopus.xy` T8 P1, `bond.xy` scene 1, OS 1.1.25 notes |
...
## Footer (firmware-dependent; after Track 16 and ending at EOF)

On the `0x11`/`0x13` family, 14 baseline one-scene slots occupy 56 bytes;
variable scene counts grow or shrink individual slots. Older families use
different baseline slot widths, so the footer must be located after Track 16
rather than by subtracting a fixed size.
