...
| Quantization | `+0x07` | raw byte; UI display is `floor(raw * 100 / 255)`. Device-authored anchors include UI 0/1/2/25/50/75/98/99/100 as `00/04/07/41/81/C0/FC/FE/FF`; PC-generated probes confirmed top-end bucket boundaries, including `FD -> 99` | `bar-q-*`, `2026-06-quant-generated/` |
...
Quantization follow-up (`src/bar-menu-probes/2026-06-quant-generated/`):
PC-authored raw-byte probes were loaded on device. All filename expectations
matched after changing the display hypothesis from `/256` to `/255`; the key
falsifier was raw `0xFD`, which displays `99` rather than `98`.
...
- Writer setters: `ImageProject.set_default_step_length_ticks`,
  `set_track_quantization_raw`, `set_track_quantization_ui`,
  `set_track_groove_raw`,
