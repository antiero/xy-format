<script lang="ts">
  ...
  $: unknownScales = project.validation.filter(
    (issue) =>
      issue.code === "track-scale-unknown" ||
      issue.code === "track-scale-read-only",
  );
</script>
...
<section class="workspace inspect-workspace">
  <div class="workspace-head">
    ...
    <div class="status-strip">
      <span
        >{project.imageProject.image.length.toLocaleString()} decoded bytes</span
      >
  ...
  <div class="inspect-grid">
    <section class="section-band">
      ...
      <dl class="kv-list">
        <div>
          <dt>steps</dt>
          <dd>{activePattern.totalSteps} raw {activePattern.rawSteps}</dd>
        </div>
        <div>
          <dt>bars</dt>
          <dd>{activePattern.bars}, final {activePattern.finalBarSteps}</dd>
        </div>
        <div>
          <dt>scale byte</dt>
          <dd>
            0x{activePattern.trackScaleRaw.toString(16).padStart(2, "0")} · {activePattern.trackScaleLabel}
          </dd>
        </div>
        <div>
          <dt>length</dt>
          <dd>{display16thsAsBars(activePattern.effectiveLength16ths)}</dd>
        </div>
        <div>
          <dt>notes</dt>
          <dd>{activePattern.notes.length}</dd>
        </div>
      </dl>
    </section>
    ...
    <section class="section-band">
      ...
      <dl class="kv-list">
        <div>
          <dt>present</dt>
          <dd>{activeScene.present ? "yes" : "no"}</dd>
        </div>
        <div>
          <dt>length</dt>
          <dd>{display16thsAsBars(activeScene.length16ths)}</dd>
        </div>
        <div>
          <dt>muted tracks</dt>
          <dd>
            {activeScene.mutedTracks
              .map((muted, i) => (muted ? `T${i + 1}` : ""))
              .filter(Boolean)
              .join(", ") || "none"}
          </dd>
        </div>
        <div>
          <dt>pattern row</dt>
          <dd>
            {activeScene.patternByTrack.map((p) => `P${p + 1}`).join(" ")}
          </dd>
        </div>
      </dl>
    </section>
    ...
    <section class="section-band">
      ...
      <p class="mono-block">
        {presentSceneFlags.length ? presentSceneFlags.join(", ") : "none"}
      </p>
    </section>
    ...
    <section class="section-band">
      ...
      <dl class="kv-list">
        <div>
          <dt>supported</dt>
          <dd>{project.songs[0]?.supported ? "yes" : "no"}</dd>
        </div>
        <div>
          <dt>loop</dt>
          <dd>{project.songs[0]?.loop ? "on" : "off"}</dd>
        </div>
        <div>
          <dt>chain</dt>
          <dd>
    ...
    <section class="section-band wide">
      <div class="section-title">
        ...
        <span
          >{unknownScales.length} timing warning{unknownScales.length === 1
            ? ""
            : "s"}</span
        >
      </div>
