...
import {
  outputMidiChannelForTrack,
  outputMidiNoteForTrack,
} from "./xy/drumMidi";
...
class AudioService {
  ...
  async init() {
    ...
    this.initializationPromise = (async () => {
      ...
      await this.context.audioWorklet.addModule(
        `${basePath}spessasynth_processor.min.js`,
      );
  ...
  noteOn(channel: number, note: number, velocity: number, delayMs: number = 0) {
    ...
    const outChannel = outputMidiChannelForTrack(channel);
    const outNote = outputMidiNoteForTrack(channel, note);
