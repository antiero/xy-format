import type { XYPatternViewModel, XYSceneViewModel, XYTrackScale, XYTrackViewModel } from './projectViewModel';
...
export const TRACK_SCALE_TO_16THS_PER_STEP: Record<Exclude<XYTrackScale, 'unknown'>, number> = {
  '1/2': 0.5,
  '1': 1,
  '2': 2,
  '3': 3,
...
export const WRITABLE_TRACK_SCALE_BYTES: Record<Exclude<XYTrackScale, 'unknown' | '3' | '4' | '6' | '8'>, number> = {
  '1/2': 0x01,
  '1': 0x03,
  '2': 0x05,
  '16': 0x0e,
...
export function decodeTrackScale(raw: number): DecodedTrackScale {
  ...
  const factor16ths = scale === 'unknown' ? null : TRACK_SCALE_TO_16THS_PER_STEP[scale];
  return {
    raw,
    scale,
    label: scale === 'unknown' ? `unknown 0x${raw.toString(16).padStart(2, '0')}` : scale,
    known: scale !== 'unknown',
    supportedForWrite: scale !== 'unknown' && Object.prototype.hasOwnProperty.call(WRITABLE_TRACK_SCALE_BYTES, scale),
    factor16ths,
  };
}
...
export function normalizeTickToPattern(rawTick: number, totalSteps: number): number {
...
export function patternEffectiveLength16ths(pattern: XYPatternViewModel): number {
...
export function noteToDisplayPosition(
  ...
  pattern: Pick<XYPatternViewModel, 'trackScale'> & Partial<Pick<XYPatternViewModel, 'totalSteps'>>
): { start16ths: number; duration16ths: number } {
  const factor = scaleTo16thsPerStep(pattern.trackScale) ?? 1;
  const tick = pattern.totalSteps ? normalizeTickToPattern(note.tick, pattern.totalSteps) : note.tick;
  return {
...
export function sceneLength16ths(scene: XYSceneViewModel, tracks: XYTrackViewModel[]): number {
  return Math.max(
    0,
