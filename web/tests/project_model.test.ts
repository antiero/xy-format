...
import { applyEdit, buildProjectViewModel } from '../src/lib/xy/projectViewModel';
...
describe('project view model and edit bridge', () => {
  it('loads a real .xy project into tracks, patterns, scenes, and Song 1', () => {
    ...
    expect(project.songs[0]).toMatchObject({ supported: true, sceneChain: [0], loop: false });
  });
  ...
  it('edits notes and pattern length, then exports and reloads the changes', () => {
    ...
    expect(reloaded.tracks[0].patterns[0].notes[0]).toMatchObject({ note: 62, velocity: 77 });
  });
  ...
  it('edits scene pattern/mute state and Song 1 chain', () => {
    ...
    expect(reloaded.songs[0]).toMatchObject({ sceneChain: [0, 1, 2], loop: false, supported: true });
  });

  it('validates scene references that exceed available patterns', () => {
    const imageProject = ImageProject.fromBytes(new Uint8Array(readFileSync(BASELINE)));
    imageProject.setScenePattern(0, 1, 8);
    const project = buildProjectViewModel(imageProject, 'bad-scene.xy');
    expect(project.validation.some((issue) => issue.code === 'scene-missing-pattern')).toBe(true);
  });
  ...
  it('reads observed scale-3 byte without falling back to unknown timing', () => {
    ...
    expect(project.validation.some((issue) => issue.code === 'track-scale-unknown')).toBe(false);
  });

  it('blocks track-scale writes whose raw bytes are not decoded', () => {
    const project = loadBaseline();
    expect(() => applyEdit(project, {
