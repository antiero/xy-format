...
function makeTrackFromAbsolute(events: Array<{ tick: number; event: Omit<MidiEvent, 'deltaTime'> }>): MidiEvent[] {
  const sorted = [...events].sort((a, b) => (
    a.tick - b.tick ||
    (a.event.type === 'noteOff' ? -1 : 1)
  ));
  let lastTick = 0;
...
function singlePolyphonicLaneMidi(): Uint8Array {
  const tpb = 480;
  const bar = tpb * 4;
  const noteEvents: Array<{ tick: number; event: Omit<MidiEvent, 'deltaTime'> }> = [];
  const voicings = [[60, 64, 67], [57, 60, 64], [62, 65, 69], [55, 59, 62]];
  ...
  for (let b = 0; b < 8; b++) {
    for (const beat of [0, 2]) {
      ...
      for (const pitch of voicings[b % voicings.length]) {
        noteEvents.push({
          tick: onset,
          event: { type: 'noteOn', channel: 7, noteNumber: pitch, velocity: 86 },
        });
        noteEvents.push({
          tick: onset + tpb * 2,
          event: { type: 'noteOff', channel: 7, noteNumber: pitch, velocity: 0 },
        });
      }
    }
  }
  ...
  const data: MidiData = {
    ...
    tracks: [
      [
        { deltaTime: 0, type: 'setTempo', meta: true, microsecondsPerBeat: Math.round(60_000_000 / 98) },
...
describe('MIDI new-project importer', () => {
  it('segments a single polyphonic lane into chord-track patterns without duplicating a second chord slot', () => {
    ...
    const result = buildMidiProjectFromBytes(singlePolyphonicLaneMidi(), 'single-chord.mid', baseline, {
      bpmOverride: 101.5,
    });
