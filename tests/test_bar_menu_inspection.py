...
def test_bar_menu_edit_reloads_with_all_decoded_track_starts() -> None:
    ...
    assert len(reloaded._starts) == 16
    assert reloaded.track_start(1) == 0x0D79
    report = generate_report(Path("bar-edit.xy"), encoded)
    assert "[Bar Menu]" in report
    assert "length_ticks=480" in report
