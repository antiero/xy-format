...
def analyze(corpus: Path) -> str:
    ...
    sections.extend(
        [
            ...
            "",
            "- `engine.transpose` has only two paired values (`0` and `12`), so the raw words are known but the general encoding is not.",
            "- `engine.tuning[]` appears in 25 presets but always with the same 12-value table; no direct float32/float64/int-cent/q16 table was found in the paired track images.",
